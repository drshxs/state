    ...
    if distanceESPEnabled then
        for _, player in pairs(game.Players:GetPlayers()) do
            if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if humanoidRootPart then
                    local studDistance = (localPlayer.Character.HumanoidRootPart.Position - humanoidRootPart.Position).Magnitude
                    local distance = studDistance / 2.617
...
local function UpdateHeadESP()
    if headESPEnabled then
        for _, player in pairs(game.Players:GetPlayers()) do
            if player ~= localPlayer and player.Character and player.Character:FindFirstChild("Head") and localPlayer.Character then
                ...
                if head and localHumanoidRootPart then
                    local headPos, onScreen = camera:WorldToViewportPoint(head.Position)

                    if onScreen then
                        local distance = (localHumanoidRootPart.Position - head.Position).Magnitude
